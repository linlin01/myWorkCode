...
public class WidgetPreviewLoader {
            ...
            if (mLoadedPreviews.containsKey(name)) {
                ...
                if (b == null){
                	if (LauncherLog.DEBUG) {
                        LauncherLog.d(TAG, "recycleBitmap() b == null");
                    }
                	return;
                } 
    ...
    static class CacheDb extends SQLiteOpenHelper {
        ...
        public CacheDb(Context context) {
            ...
            mContext = context;
            if (LauncherLog.DEBUG) {
                LauncherLog.d(TAG, "CacheDb: ctor, cache dir = " + new File(context.getCacheDir(), DB_NAME).getPath());
            }
        }
        ...
        public void onCreate(SQLiteDatabase database) {
            database.execSQL("CREATE TABLE IF NOT EXISTS " + TABLE_NAME + " (" +
                    ...
                    ");");
            if (LauncherLog.DEBUG) {
                LauncherLog.d(TAG, "onCreate: create database");
            }
        }
    ...
    private void writeToDb(Object o, Bitmap preview) {
        ...
        if (LauncherLog.DEBUG) {
            LauncherLog.d(TAG, "writeToDb: db = " + db);
        }
    ...
    public static void removePackageFromDb(final CacheDb cacheDb, final String packageName) {
        ...
        new AsyncTask<Void, Void, Void>() {
            public Void doInBackground(Void ... args) {
                ...
                if (LauncherLog.DEBUG) {
                    LauncherLog.d(TAG, "removeFromDb: db = " + db);
                }
                try {
    ...
    public Bitmap generateWidgetPreview(AppWidgetProviderInfo info, int cellHSpan, int cellVSpan,
        ...
        if (widgetPreviewExists) {
            ...
            if (previewWidth <= 0 || previewHeight <= 0) {
                if (LauncherLog.DEBUG) {
                    LauncherLog.d(TAG, "getWidgetPreview: packageName = " + packageName +
                        ", getIntrinsicWidth() = " + previewWidth +
                        ", getIntrinsicHeight() = " + previewHeight);
                }
            }
        } else {
		...
		//add  JWYYL-104  xujie  20141216(start)
		if (previewHeight * scale > maxPreviewHeight) {
            scale = maxPreviewHeight / ((float) previewHeight + 15);
        }
		//add  JWYYL-104  xujie  20141216(end)
