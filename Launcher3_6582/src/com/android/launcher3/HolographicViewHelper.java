...
public class HolographicViewHelper {
    ...
    void generatePressedFocusedStates(ImageView v) {
        if (!mStatesUpdated && v != null) {
            ...
            //outlineD = new FastBitmapDrawable(outline);
            //hotwordD = new FastBitmapDrawable(hotword);

			WeakReference<FastBitmapDrawable> weekOriginalD = new WeakReference<FastBitmapDrawable>(new FastBitmapDrawable(original));
			WeakReference<FastBitmapDrawable> weekOutLineD = new WeakReference<FastBitmapDrawable>(new FastBitmapDrawable(original));
			WeakReference<FastBitmapDrawable> weekHotwordD = new WeakReference<FastBitmapDrawable>(new FastBitmapDrawable(original));

			StateListDrawable states = new StateListDrawable();
			states.addState(new int[] {android.R.attr.state_pressed}, weekOutLineD.get());
            states.addState(new int[] {android.R.attr.state_focused}, weekOutLineD.get());
            states.addState(new int[] {R.attr.stateHotwordOn}, weekHotwordD.get());
            states.addState(new int[] {}, weekOriginalD.get());  
			/// M.
