...
public class DeviceProfile {
    ...
    private void updateIconSize(float scale, int drawablePadding, Resources resources,
        ...
        cellHeightPx = iconSizePx + 9 * iconDrawablePaddingPx + (int) Math.ceil(fm.bottom - fm.top);//modify  JWYYL-126  xujie  20141216
...
//modify by liuxianbang for Ani 20150203 (start)
        ViewGroup screenEffects = launcher.getScreenEffects();
...
//add DSSLE-56 liuxianbang 20140205 (start)
            if(!ViewConfiguration.get(launcher).hasPermanentMenuKey()){
              int navBarHeight = res.getDimensionPixelSize(com.android.internal.R.dimen.navigation_bar_height);
              LauncherLog.d(LauncherLog.TAG_ANI,"no hard menu,and navbarheight  is:"+navBarHeight);
              lp.setMargins(0,0,0,-navBarHeight);
            }
//add DSSLE-56 liuxianbang 20140205 (end)
            overviewMode.setLayoutParams(lp);
            if (screenEffects != null) {
                screenEffects.setLayoutParams(lp);
            }
